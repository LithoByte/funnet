...
class MultipartTests: XCTestCase {
  ...
  func testURLConvertible() {
    let encoder = FormDataEncoder()
    let url = Bundle.main.url(forResource: "0", withExtension: ".jpg")
    print(url)
    let data = try? Data(contentsOf: url!)
    let image = try? UIImage(data: data!)
    print(image)
    let user = AccountInfo(firstName: "Calvin", lastName: "Collins", phoneNumber: "5037899196", email: "cjc8@williams.edu", password: "password", passwordConfirmation: "password", invitationToken: "token", data: nil, avatarURL: url)
    let boundary = "--boundary-pds-site\(Date().timeIntervalSince1970)file-image-boundary--"
